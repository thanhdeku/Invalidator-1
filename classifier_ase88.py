from lib import utils_ase as ut
from tqdm import tqdm
import os
from lib.utils_ase import read_idx2id


def print_info(patch):
    try:
        project, bug_id, correctness, tool, _ = ut.read_info_patch(patch)
    except:
        project, bug_id, correctness, tool, _ = ut.read_info_patch(patch)
...
def classifier(cf_threshold, softrules = True, hardrules = True, learning = True, only_learning = False):
    ...
    total_correct = 0
    total_incorrect = 0
    path = "/Users/thanhdeku/Research/Data/patchValidation/confidence_score/patches"
    ...
    for patch in list_patches:
        ...
        # distance2incorrect, distance2correct = distance2incorrect/(distance2incorrect + distance2correct), distance2correct/(distance2incorrect + distance2correct)
        confidence_invariant = distance2correct - distance2incorrect
        confidence_learning = learning_prob[patch][1] - learning_prob[patch][0]

        # if confidence_invariant > 0 and confidence_learning > 0:
        #     rs = True
        # else:
        if softrules:
            if learning:
        ...
        if only_learning:
            rs = False
            if learning_prob[patch][1] > cf_threshold:
                    rs = True
        if rs is not None:
            ...
            if rs:
                print("Patch: ", patch, " Correctness: ", correctness)
                if correctness == "Incorrect":
                    rs4project_tp[project] += 1
                    list_incorrect.append(patch)
                    true_overfit += 1
                elif correctness == "Correct" or correctness == "Same":
                    rs4project_fp[project] += 1
                    list_correct.append(patch)
                    false_overfit += 1
